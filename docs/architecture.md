...
| `WebhookHandlerBase` | `server/webhook-handler-base.ts` | Abstract base for webhook handlers — event ring buffer, processing watchdog, status tracking |
...
| `CommitEventHandler` | `server/commit-event-handler.ts` | Git post-commit hook → commit-review workflow dispatch (filter chain + dedup) |
...
| `SessionRestartScheduler` | `server/session-restart-scheduler.ts` | Pure restart decision logic — evaluates crash restart eligibility (count, cooldown, user-stop) |
